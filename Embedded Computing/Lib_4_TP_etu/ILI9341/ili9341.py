...
class Display(object):
    ...
    def draw_text(self, x, y, text, font, color,  background_flag = True, background=0,
    ...
    def draw_text2(self, x, y, text, text_prec, font, color,  background_flag = True, background=0,
                  landscape = False, spacing = 1):
        """Draw text.

        text et text_prec : chaines de caractéres de même taille
        Redessiner uniquement les caractéres de text de même rang dans text et text_prec
            qui sont différents au sein de text et text_prec

        Args:
            x (int): Starting X position.
            y (int): Starting Y position.
            text (string): Text to draw.
            text_prec (string): Text to draw.
            font (XglcdFont object): Font.
            color (int): RGB565 color value.
            background (int): RGB565 background color (default: black).
            landscape (bool): Orientation (default: False = portrait)
        ...
        for i in range (len(text)):
            # Get letter array and letter dimensions
            if text[i] == text_prec[i] :
                w, h = self.draw_letter(x, y, text[i], font, color, background, landscape)
            else :
                w, h = self.draw_letter(x, y, text_prec[i], font, 0, background, landscape)
                w, h = self.draw_letter(x, y, text[i], font, color, background, landscape)
            # Stop on error
            if w == 0 or h == 0:
                print('Invalid width {0} or height {1}'.format(w, h))
