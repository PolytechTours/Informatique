...
Modification pour Raspberry Pi pico et Micropython
* Author(s): PG

**Hardware:**
* `Hardware : Adafruit SGP40 Air Quality Sensor Breakout - VOC Index <https://www.adafruit.com/product/4829>`_
...
class SGP40:
    ...
    **Quickstart: Importing and using the SGP40 temperature sensor**
    ...
    def __init__(self, i2c_bus, address = SGP40_I2C_ADDR):
    ...
    def initialize(self):
        ...
        self._serial_number = self._read_word_from_command(readlen=3)
        #serialnumber = self._read_word_from_command(3)
        #if serialnumber[0] != 0x0000:
        # print (self._serial_number) pour vérification
        if self._serial_number[0] != 0x0000:
    ...
    def _read_word_from_command(self, delay_ms=10, readlen=1,):
        ...
        '''
        Ne fonctionne pas sur Raspberry Pi pico et micropython
        with self.i2c_device as i2c:
            i2c.write(self._command_buffer)
        '''
        self.i2c_device.writeto(SGP40_I2C_ADDR, self._command_buffer, True)
        ...
        '''
        self.i2c_device.readfrom_into(SGP40_I2C_ADDR, replybuffer, True)
